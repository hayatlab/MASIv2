...
class Classifier(nn.Module):
    def __init__(self,marker_dim=20,output_dim=16,t2=1.0):
    ...
    def forward(self, x):
        ...
        f = torch.reshape(x, (x.shape[0],self.marker_dim, self.output_dim))
...
def train_classifier(X, model,epochs,pseudoy):
    ...
    for e in range(epochs):
        ...
        _,out = model(noise_inputs)

        loss = nn.NLLLoss()(torch.log(out),labels)
