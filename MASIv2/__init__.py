...
def gene2mat(
        ad=None, cell_markers=None, thresh=0.5, if_tfidf=True, if_thresh=True
):
    # TF-IDF transformation
    X = ad.X.copy()
    if scipy.sparse.issparse(X):
...
'''
class GraphClassifier_noref(nn.Module):
    def __init__(self, marker_dim=20, output_dim=16, t1=1.0, temp=1.0):
        super(GraphClassifier_noref, self).__init__()
        self.output_dim = output_dim
        self.marker_dim = marker_dim
        self.markerencoder = MarkerWeight(marker_dim=self.marker_dim, output_dim=self.output_dim)
        self.gc = AGNNConv(allow_zero_in_degree=True,learn_beta=False, temp=temp)
        self.batchnorm = nn.BatchNorm1d(self.output_dim)
        self.activation = nn.LeakyReLU(0.25)
        self.softmax = nn.Softmax(dim=1)
        self.t1 = torch.tensor(t1, requires_grad=False)
...
class GraphClassifier(nn.Module):
    def __init__(self, marker_dim=20, output_dim=16, t1=5.0, t2=0.1, temp=1.0, ref_center = None):
    ...
    def forward(self, g, x):
        ...
        p = F.softmax(f/self.t2, 1)
        p=self.centroid(p)
        
        out = self.gc(g, f)
        out = self.gc(g, out)
        out = F.softmax(out/self.t1, 1)
...
def parallelAGN_noref(
    ...
    for i in range(len(ad_list)):
        ...
        student = GraphClassifier_noref(output_dim=labels.shape[1], marker_dim=20, t1=t1, temp=temp)
...
def parallelAGN(
    ...
    for i in range(len(ad_list)):
        ...
        student = GraphClassifier(output_dim=labels.shape[1], marker_dim=marker_dim,
                                  t1=t1, t2=t2, temp=temp, ref_center=ref_center)
...
def reg_align(source, target):
    ...
    for c in cell_type[1:]:
        source_input = np.concatenate((source_input, 
                                       np.expand_dims(source[source.obs['cell_type'] == c].obsm['X_score'][0, :], axis=1)), axis=1)
        target_input = np.concatenate((target_input, 
                                       np.expand_dims(target[target.obs['cell_type'] == c].obsm['X_score'][0, :], axis=1)), axis=1)
...
def find_key_cell(
        source=None, batch_size=50000, res=2.0, key_node_metric='degree centrality'
):
